...
from nnunetv2.WaveletTransform3D import *
from .nnUNetTrainer import *
def my_get_dice_loss(preds: torch.tensor, target: torch.tensor):
    pred = torch.softmax(preds, dim=1)[:, 1, ...].unsqueeze(1)  # 取出目标通道
    inter = (pred * target).sum(dim=(2, 3, 4))  # 计算交集
    union = (pred + target).sum(dim=(2, 3, 4))  # 计算并集

    dice_loss = 1 - (2.0 * inter + 1) / (union + 1)  # 添加平滑因子
    return dice_loss.mean()
class NewnnUNetTrainer(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        super(NewnnUNetTrainer, self).__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # ).to(self.device)
            self.num_epochs = 200 #max_epoch
            self.network = WaveletUNet3D(
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # loss = self.loss(output, target[0])  # 只取 target[0]
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # print("开始计算验证损失")
            l = self.loss(output, target[0])
            # print("before enable_deep_supervision output.shape",output.shape)
            # l = self.loss(output, target[0])  # 只取 target[0]
        # print("before enable_deep_supervision target.shape",target.shape)
        ...
        if self.label_manager.has_regions:
            ...
            del output_seg
        # print(f"before self.label_manager.has_ignore_labelTarget shape: {target.shape}")
