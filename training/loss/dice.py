...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    with torch.no_grad():
        ...
        if net_output.shape == gt.shape:
            ...
            # 初始化 y_onehot，确保其形状正确
            num_classes = net_output.shape[1]
            y_onehot = torch.zeros_like(net_output, device=net_output.device, dtype=torch.bool)

            # 检查 gt 的形状是否正确，若必要则调整
            if gt.ndim == 4:  # 如果 gt 是 [N, D, H, W]
                gt = gt.unsqueeze(1)  # 添加类别维，变为 [N, 1, D, H, W]

            # 确保 gt 的类别值不会超过 num_classes
            if gt.max() >= num_classes:
                raise ValueError(f"gt contains invalid class indices (>= {num_classes}).")
            # print(f"gt.shape: {gt.shape}, num_classes: {num_classes}")
            # print("y_onehot.shape",y_onehot.shape)
            # 生成 one-hot 编码
            y_onehot.scatter_(1, gt.long(), 1)
