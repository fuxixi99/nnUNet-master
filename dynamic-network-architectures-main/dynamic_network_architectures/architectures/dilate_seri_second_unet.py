...
class FEM_Series(nn.Module):  # 并联的空洞卷积块
    def __init__(self, in_planes, out_planes, dilation_rates=[1, 2]):
        ...
        self.branches.append(
            nn.Sequential(
                BasicConv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1),
                BasicConv(out_planes, out_planes, kernel_size=3, stride=1, padding=4, dilation=4)
        ))
        # 最后的1x1卷积用于融合所有分支
        self.ConvLinear = BasicConv(
            (len(dilation_rates)+1) * out_planes, out_planes, kernel_size=1, stride=1, relu=False
        )
...
class DilateSeriSecondPlainConvUNet(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
...
if __name__ == "__main__":
    ...
    model = DilateSeriSecondPlainConvUNet(
        1,
        6,
