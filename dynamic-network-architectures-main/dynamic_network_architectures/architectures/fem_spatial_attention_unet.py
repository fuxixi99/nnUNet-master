...
class FEM(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, map_reduce=8):
        ...
        self.channel_attention = nn.Sequential(
            ...
            nn.Conv3d(
                inter_planes * 3 * 2, inter_planes * 2, kernel_size=1, bias=False
            ),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                inter_planes * 2, inter_planes * 3 * 2, kernel_size=1, bias=False
            ),
            nn.Sigmoid(),
        )
...
class FemSpatialAttentionPlainConvUNet(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
...
if __name__ == "__main__":
    ...
    model = FemSpatialAttentionPlainConvUNet(
        1,
        6,
