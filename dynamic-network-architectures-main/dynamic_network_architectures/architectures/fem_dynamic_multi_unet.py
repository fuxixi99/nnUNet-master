...
class FemDynamicMultiPlainConvUNet(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
    ...
    def forward(self, x):
        ...
        # 增强 skip 连接
        enhanced_skips = []
        for skip, fem, dyenhance in zip(skips, self.FEM, self.dynamic_net):
            skip = skip.float()  # 确保输入为 float32
            ll = fem(skip)  # 应用 FEM 模块
            feature = dyenhance(skip)
            # enhanced_skip = torch.cat([skip, feature], dim=1)
...
if __name__ == "__main__":
    ...
    model = FemDynamicMultiPlainConvUNet(
        1,
        6,
