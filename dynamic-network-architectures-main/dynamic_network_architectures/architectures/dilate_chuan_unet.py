...
class FEM_Series(nn.Module):  # 串联的空洞卷积块
...
class FEM_Series_Sequential(nn.Module):  # 串联的空洞卷积块（串联版本）
    def __init__(self, in_planes, out_planes, dilation_rates=[1, 2, 4]):
        super(FEM_Series_Sequential, self).__init__()
        layers = []
        current_in = in_planes
        # 按序依次添加各个空洞卷积层，每层使用不同的膨胀率
        for dilation in dilation_rates:
            layers.append(
                BasicConv(
                    current_in,
            ...
            current_in = out_planes  # 更新下一层的输入通道数
        self.sequence = nn.Sequential(*layers)
    
    def forward(self, x):
        return self.sequence(x)
    
class DilateChuanPlainConvUNet(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
        ...
        self.FEM = nn.ModuleList(
            [FEM_Series_Sequential(features, features) for features in self.encoder.output_channels]
        )
...
if __name__ == "__main__":
    ...
    model = DilateChuanPlainConvUNet(
        1,
        6,
