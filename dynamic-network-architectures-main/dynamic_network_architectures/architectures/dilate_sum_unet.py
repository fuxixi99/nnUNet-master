...
class FEM_Series(nn.Module):  # 并联的空洞卷积块
    ...
    def forward(self, x):
        ...
        # 对每个分支输出进行累加，形成 [x0, x0+x1, x0+x1+x2, x0+x1+x2+x3]
        cumulative_outputs = []
        cumulative = None
        for out in branch_outputs:
            if cumulative is None:
                ...
                cumulative = cumulative + out
            cumulative_outputs.append(cumulative)

        # 将累加后的输出在通道维度上进行拼接
        out = torch.cat(cumulative_outputs, 1)
...
class DilateSumPlainConvUNet(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
