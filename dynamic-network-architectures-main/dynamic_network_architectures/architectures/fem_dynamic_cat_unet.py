...
def load_nii_image_as_tensor(file_path, device="cuda"):
    ...
    image_tensor = image_tensor.unsqueeze(0).unsqueeze(
        0
...
class FemDynamicCatPlainConvUNet(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
        ...
        self.dynamic_net = nn.ModuleList(
            [
                DynamicPositioningAttention3D(in_channels=features)
                for features in self.encoder.output_channels
            ]
        )
        self.fuse_conv = nn.ModuleList([nn.Conv3d(3 * features, features, kernel_size=1, stride=1, padding=0)for features in self.encoder.output_channels])
    ...
    def forward(self, x):
        ...
        # 增强 skip 连接
        enhanced_skips = []
        for skip, fem, dyenhance, fuse_conv in zip(skips, self.FEM, self.dynamic_net,self.fuse_conv):
            skip = skip.float()  # 确保输入为 float32
            ll = fem(skip)  # 应用 FEM 模块
            feature = dyenhance(skip)
            enhanced_skip = torch.cat([skip,ll, feature], dim=1)
            enhanced_skip = fuse_conv(enhanced_skip)
            # enhanced_skip = skip + ll +feature
            # 卷积层，输入通道是enhanced_skip的通道数，输出通道是输入的一般
            # conv=nn.Conv3d(enhanced_skip.shape[1], enhanced_skip.shape[1] // 2, kernel_size=3, stride=1, padding=1)
            # enhanced_skip=conv(enhanced_skip)
...
if __name__ == "__main__":
    ...
    model = FemDynamicCatPlainConvUNet(
        1,
        6,
