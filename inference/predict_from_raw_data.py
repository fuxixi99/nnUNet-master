...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(
        ...
        for i, f in enumerate(use_folds):
            ...
            print(
                f"Loading fold {f} from {model_training_output_dir} in {checkpoint_name}"
            )
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(prediction, list) and len(prediction) == 1:
            prediction = prediction[0]  # 提取 list 的第一个元素
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction_test = self.network(torch.flip(x, axes))
                if isinstance(prediction_test, list):
                    # print("List length:", len(prediction_test))
                    # print("Element shapes:", [p.shape for p in prediction_test])
                    prediction_test = prediction_test[0]
                prediction += torch.flip(prediction_test, axes)
